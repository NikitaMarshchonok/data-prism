...
The browser workflow submits `POST /vibedash/comparisons/jobs`, polls the scoped asynchronous job at `/vibedash/jobs/<job_id>`, and opens `/vibedash/jobs/<job_id>/result`; the aggregate manifest is available at `/vibedash/jobs/<job_id>/manifest`. Job, history, result, and manifest access is restricted to the signed browser session that created the job.
